...
### Fixed
- **Bill image me logo / photo / QR / barcode nahi dikhte the (text black theek tha)** — WhatsApp share, PNG download aur preview teeno me. html-to-image bill ko SVG `<foreignObject>` ke through paint karta hai; WebKit (iPhone/Safari) aur kabhi-kabhi Chrome us raste `<img>` ko chhod deta hai — text aa jaata hai, tasveerein gayab. Ab text paint hone ke baad har `<img>` (logo, cloned header/footer photo, UPI QR, barcode SVG) ko hum khud canvas par uski exact jagah draw karte hain, seedha decoded element se. Sabhi images data-URI hain isliye canvas taint bhi nahi hota.
- **Bill image blank / white aati thi** ("Image ke roop me" share, PNG download, preview) — capture node `position:fixed; left:-99999px` par tha; html-to-image us computed style ko clone par copy karta hai, to SVG foreignObject me content viewport ke bahar render hota tha aur sirf white background bachta tha. Ab off-screen wrapper alag hai, captured root static hai, clone ko `style` reset milta hai, fonts/paint frames ka wait hota hai, aur agar phir bhi blank PNG bane to pixel-check karke plain-canvas fallback chal jaata hai.
### Added
- **Mithai / Halwai 80mm bill template** (Settings → Bill Templates → Thermal) — Kali Ghata-style sweets-shop receipt: CIN + GSTIN + FSSAI header, `Bill No / Bill Type / Cashier / No of Items / Time` block, 3-decimal quantity (0.250 / 1.000 kg), HSN under every item, `Net Qty / Bill Total / Round Off / Payable Amt`, `Tender Amount / Balance Amount`, rate-wise **GST SUMMARY** (5% / 12% / 18% → taxable, CGST, SGST) and a big boxed **Token No.** Auto-selected when the Sweets / Mithai system is activated.
- **CIN field** under Settings → Business → Legal identifiers (also auto-filled by bill OCR). New template tokens: `{{cin}}`, `{{token_no}}`, `{{qty3}}`, `{{qty_total3}}`, `{{#gst_summary}}…{{/gst_summary}}`, `{{gst_sum_*}}`, per-line `{{taxable}} {{cgst_amt}} {{sgst_amt}}`.
