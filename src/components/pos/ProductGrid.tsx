...
export const ProductRow = memo(function ProductRow({ p, onPick, onLong }: { p: Product; onPick: (p: Product) => void; onLong?: (p: Product) => void }) {
  const s = useSettings();
  const st = stockState(p);
  return (
    <button onClick={() => onPick(p)} onContextMenu={(e) => { e.preventDefault(); onLong?.(p); }}
      className="flex h-full w-full items-center gap-3 rounded-xl border border-line bg-surface px-3 py-2.5 text-left transition hover:border-brand/50 active:scale-[.99]">
...
export const ProductCard = memo(function ProductCard({ p, onPick, onLong }: { p: Product; onPick: (p: Product) => void; onLong?: (p: Product) => void }) {
  const s = useSettings();
  const st = stockState(p);
  return (
    <button onClick={() => onPick(p)} onContextMenu={(e) => { e.preventDefault(); onLong?.(p); }}
      className={cx('group relative flex h-full w-full flex-col overflow-hidden rounded-xl border bg-surface p-2.5 text-left transition active:scale-[.97]',
